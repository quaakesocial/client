<script lang="ts">
  ...
  async function lovePost() {
    let res;
    if(post.loves.includes(username)) {
...
<div class="bg-[#15151A] w-[26em] rounded-lg p-10">
  ...
  <br>
  <button class="flex items-center gap-2 bg-inherit outline-inherit border-none p-0 hover:bg-inherit" title={`loved by ${post.loves.join(', ')}`} on:click={async () => await lovePost()}>
  {#if post.loves.includes(username)}
